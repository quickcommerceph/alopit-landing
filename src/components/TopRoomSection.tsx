...
export function TopRoomSection() {
  ...
  return (
    <section
      id="top-room"
      className="mx-auto max-w-7xl scroll-mt-24 px-4 pb-16 pt-6 sm:px-6 lg:px-8"
    >
      <motion.div
      ...
      >
        <div>
          ...
          <h2 className="mt-2 text-3xl font-black tracking-tight text-primary sm:text-4xl">
            Top Room of the Night
          </h2>
          <p className="mt-3 max-w-xl text-sm font-medium leading-6 text-secondary sm:text-base">
            The fastest-moving broadcast right now — biggest crowd, hottest card, prime-time pit.
          </p>
        </div>
        ...
        <div className="flex w-full flex-wrap gap-2 sm:w-auto sm:justify-end">
          <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-bold text-secondary ring-1 ring-border">
            ...
            {spotlight.momentum}% momentum
          </span>
          <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-bold text-secondary ring-1 ring-border">
            <CalendarClock className="h-3.5 w-3.5 text-accent" />
            Starts {spotlight.startTime}
          </span>
        </div>
      </motion.div>
        ...
        <div className="grid lg:grid-cols-[0.95fr_1.05fr]">
          <div className="relative min-h-[280px] overflow-hidden bg-[#0b1020] sm:min-h-[360px] lg:min-h-[560px]">
            <img
              ...
              alt={`${spotlight.title} live broadcast arena`}
            ...
            <motion.div {...fadeUp(0.2)} className="absolute bottom-5 left-5 right-5">
              ...
              <p className="mt-4 text-xs font-black uppercase tracking-[0.22em] text-white/70">
                Featured card · {spotlight.round}
              </p>
              <h3 className="mt-2 max-w-xl text-3xl font-black leading-tight tracking-tight text-white sm:text-4xl lg:text-5xl">
                {spotlight.title}
              </h3>
              <p className="mt-4 max-w-xl text-sm font-medium leading-6 text-white/78 sm:text-base">
                {spotlight.summary} Stream live in {spotlight.quality.toLowerCase()} —
                jump in before the next round starts.
              </p>
          ...
          <div className="flex min-w-0 flex-col p-6 sm:p-7 lg:p-8">
            ...
            <motion.div {...fadeUp(0.15)} className="mt-7">
              ...
              <p className="mt-4 max-w-xl text-sm font-medium leading-6 text-secondary sm:text-base">
                Tracked live across crowd, handle, and entry windows. The fastest-moving
                broadcast right now — and the easiest seat to claim.
              </p>
            </motion.div>
              ...
              <div className="mt-5">
                ...
                <div className="h-2 overflow-hidden rounded-full bg-bg-alt ring-1 ring-border-light">
                  <motion.div
                    ...
                    whileInView={{ width: `${spotlight.momentum}%` }}
                    viewport={{ once: true }}
                    transition={
                      prefersReducedMotion
                        ? { duration: 0 }
